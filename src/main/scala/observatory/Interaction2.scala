...
object Interaction2 extends Interaction2Interface {
  ...
  def yearSelection(selectedLayer: Signal[Layer], sliderValue: Signal[Year]): Signal[Year] = {
    Signal[Year] {
      val sliderSignalValue = sliderValue()
      val boundsSignalValue = yearBounds(selectedLayer)()
      if (sliderSignalValue < boundsSignalValue.min) boundsSignalValue.min
      else if (sliderSignalValue > boundsSignalValue.max) boundsSignalValue.max
      else sliderSignalValue
    }
  }
  ...
  def caption(selectedLayer: Signal[Layer], selectedYear: Signal[Year]): Signal[String] = {
    Signal[String] {
      selectedLayer().layerName + " (" + selectedYear() + ")"
    }
  }
